...
All symbols meant for public use must start with `curl`. Global internal
symbols start with `Curl`.
...
Source code in curl may never be wider than 79 columns. There are two reasons
...
2. Narrower columns allow developers to more easily view multiple pieces of
   code next to each other in different windows. I often have two or three
   source code windows next to each other on the same screen, as well as
   multiple terminal and debugging windows.
